...
sequenceDiagram
    ...
    BO->>BS: emitEventUpdate()<br/>with ODDS_UPDATE
    ...
    BO->>BS: setSuspended()<br/>with EVENT_UPDATE
    BS->>Server: socket.emit('event:update',<br/>channel, message)
    ...
    Note right of BO: - Removed useEventsSocket.ts<br/>- Added usePriceUpdate hook<br/>- Price updates debounced<br/>- Visual feedback during updates

    Note over BO,SB: Channel Format
    Note right of Server: Channel format:<br/>'*:Event:{eventId}'<br/>Example: '*:Event:1'
```