...
export const Betslip = () => {
  ...
  // Calculate totals using the actual event and selection data
  const betsWithData = bets.map(bet => {
  ...
  }).filter((bet): bet is typeof bet & { event: Event, selection: Selection } => 
    bet.event !== undefined && bet.selection !== undefined
  );

  const totalStake = betsWithData.reduce((sum, bet) => 
    bet.event.suspended ? sum : sum + (bet.stake || 0)
  , 0);
  
  const potentialWinnings = betsWithData.reduce((sum, bet) => 
  ...
  const hasActiveBets = betsWithData.some(bet => !bet.event.suspended);
